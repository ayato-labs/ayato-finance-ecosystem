...
def main():
    ...
    sync_group.add_argument(
        ...
        nargs="?",
        const="AUTO",
        help="Run full EDINET historical backfill. If path is omitted, downloads latest master from EDINET API.",
    )
    ...
    if args.edinet_only:
        ...
        sync_service.stop()
        return  # Terminate after edinet-only if no other market sync requested
    ...
    if args.sync_market:
        ...
        if market_choice in ["jp", "all"]:
            ...
            # Trigger EDINET Sync as part of JP/All market sync
    ...
    if args.edinet_backfill:
        try:
            ...
            logger.error(f"EDINET Backfill failed: {e}")

    sync_service.stop()

    if not any([args.api, args.sync, args.sync_market, args.edinet_backfill, args.edinet_only]):
        parser.print_help()
