...
class JPEngine:
    ...
    def ingest_prices(self, df: pd.DataFrame, session_id: str):
        ...
        error_count = 0
        for _, row in df.iterrows():
            # Filter: Skip records without actual trading data
        ...
        try:
            with db_manager.connect(db_path) as conn:
                ...
                # Cleanup: Ensure no Null price records exist
                conn.execute("DELETE FROM daily_prices WHERE Close IS NULL OR Volume IS NULL")
