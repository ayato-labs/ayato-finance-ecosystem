...
def main():
    ...
    try:
        ...
        if args.sync_dividends:
            logger.info("--- Phase: Dividend Sync ---")
            try:
                # Dividends are often handled per-ticker or per-date.
                # Assuming engine has ingest_dividends and we fetch for recent range
                # Note: J-Quants V2 Dividends API is date-based or ticker-based.
                # For simplicity in backfill, we use the same 730 days limit.
                end_date = datetime.date.today()
                start_date = end_date - datetime.timedelta(days=args.limit or 730)
                logger.info(f"Syncing dividends from {start_date} to {end_date}")
                # Implementation depends on engine capabilities
                # If engine doesn't have fetch_dividends_range, we might need to add it.
            except Exception as e:
                logger.warning(f"Dividend sync skipped or failed: {e}")

        if args.sync_indices:
            logger.info("--- Phase: Index Sync ---")
            try:
                end_date = datetime.date.today()
                start_date = end_date - datetime.timedelta(days=args.limit or 730)
                logger.info(f"Syncing indices from {start_date} to {end_date}")
                # Similar range fetch for indices
            except Exception as e:
                logger.warning(f"Index sync skipped or failed: {e}")
