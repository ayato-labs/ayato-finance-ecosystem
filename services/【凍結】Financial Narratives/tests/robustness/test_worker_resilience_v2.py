...
def robustness_env(tmp_path):
    return {"master": str(tmp_path / "master.sqlite"), "jp": str(tmp_path / "jp_lake.duckdb")}
...
async def test_worker_poison_pill_isolation(robustness_env):
    ...
    with (
        ...
        # DuckDBにデータを仕込む (Storage がテーブル作成済み)
        with duckdb.connect(robustness_env["jp"]) as conn:
            conn.execute(
                """
                INSERT INTO filings VALUES (
                    'SAFE-1', 'GOOD', '0', '120', '2026-05-01', '{"t":"safe"}',
                    '{}', CURRENT_TIMESTAMP
                )
                """
            )
            conn.execute(
                """
                INSERT INTO filings VALUES (
                    'POISON-1', 'BAD', '0', '120', '2026-05-01', '{"t":"POISON"}',
                    '{}', CURRENT_TIMESTAMP
                )
                """
            )
            conn.execute(
                """
                INSERT INTO filings VALUES (
                    'SAFE-2', 'GOOD', '0', '120', '2026-05-01', '{"t":"safe"}',
                    '{}', CURRENT_TIMESTAMP
                )
                """
            )
