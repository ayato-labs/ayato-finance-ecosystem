...
def test_enforce_schema_from_series():
    ...
    series = pd.Series(
        [5.25, 5.50], index=pd.to_datetime(["2024-01-01", "2024-02-01"]), name="Value"
    )
...
def test_enforce_schema_invalid_value():
    ...
    df = pd.DataFrame({"Date": ["2024-01-01"], "Value": ["invalid_rate"]})
...
def test_enforce_schema_date_normalization():
    """日付が正規化(時刻除去)されるか"""
    df = pd.DataFrame({"Date": [pd.Timestamp("2024-01-01 15:30:00")], "Value": [1.23]})
