...
class EdgarParser:
    ...
    CORE_SECTIONS: ClassVar[list[str]] = [
        "Business", "Risk Factors", "Unresolved Staff Comments", 
        "Management's Discussion and Analysis", "Quantitative and Qualitative Disclosures About Market Risk",
        "Financial Statements", "Directors, Executive Officers and Corporate Governance",
    ...
    def extract_all_sections(self, html_content: str, form_type: str) -> dict[str, str]:
        ...
        indices = []
        current_key = "header_info"
        
        for i, line in enumerate(lines):
            # 目次のリンクを除外するための簡易チェック
            if item_pattern.match(line) and len(line) < 200 and not re.search(r"\[.*\]\(#.*\)", line):
...
if __name__ == "__main__":
    parser = EdgarParser()
    test_html = "<html><body><h1>Item 1. Business</h1><p>Our business is great.</p><h1>Item 1A. Risk Factors</h1><p>Many risks.</p></body></html>"
