...
async def test_true_asynchronous_overlap_proof():
    ...
    async def mock_process_us(ticker, fetcher, parser, storage, run_structuring, days, structuring_tasks):
    ...
    with patch("src.batch_fetch.process_us_ticker", side_effect=mock_process_us):
        ...
        await sync_recent_us_filings(mock_fetcher, mock_parser, mock_storage, days=1, run_structuring=True)
        end_time = time.perf_counter()

    # 1. 実行時間の証明:
    # もし直列(await)なら: (0.1 + 0.5) * 3 = 1.8秒以上かかる
    # 並列・非同期なら: I/O(0.1*3) は約0.3秒で終わり、裏で走るStruct(0.5)は並列処理されるので、全体で約0.8〜1.0秒で終わるはず
    duration = end_time - start_time
    assert duration < 1.5, f"実行時間が長すぎます({duration:.2f}s)。非同期オーバーラップが機能していません。"

    # 2. 実行順序の証明 (非ブロッキングの証拠):
    # T1のSTRUCT_END (完了) よりも前に、T2やT3のFETCH_START (開始) がログに記録されていなければならない
    t1_struct_end_idx = event_log.index("T1_STRUCT_END")
    t2_fetch_start_idx = event_log.index("T2_FETCH_START")
    
    assert t2_fetch_start_idx < t1_struct_end_idx, "T1の構造化が終わるまでT2のフェッチがブロックされています！"
...
async def test_poison_pill_task_isolation():
    ...
    return_exceptions=True によって他の正常なタスクが巻き添えで死なず、最後まで完走することを証明する。
    """
    mock_fetcher = MagicMock()
