...
async def test_parallel_execution_timing():
    ...
    with patch("src.batch_fetch.sync_recent_jp_filings", side_effect=delayed_fetch) as mock_jp, \
        ...
        assert duration < 1.5, f"Execution took too long ({duration:.2f}s), parallelization might be broken."
...
async def test_parallel_error_isolation():
    ...
    with patch("src.batch_fetch.sync_recent_jp_filings", side_effect=fail_fetch), \
