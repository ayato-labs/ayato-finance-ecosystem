...
def audit():
    ...
    if not missing and not extra:
        print(green("  Schema Valid: Exactly 6 core dimensions + 4 metadata columns found."))
    else:
        if missing:
            print(red(f"  Missing required columns: {missing}"))
        if extra:
            print(red(f"  Non-compliant extra columns found: {extra}"))

    # 2. Catalog Integrity Audit
    print("\n[2/4] Auditing Metadata Catalog Consistency...")
    with sqlite3.connect(DB_PATH) as conn:
        db_file_count = conn.execute(
            "SELECT COUNT(DISTINCT file_path) FROM ticker_index"
        ).fetchone()[0]
        db_ticker_count = conn.execute(
    ...
    if db_file_count == fs_file_count:
        ...
        print(
            red(f"  Drift Detected! Files on disk: {fs_file_count}, Indexed in DB: {db_file_count}")
        )
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            if elapsed < 0.5:
                print(green("  Performance Check: SUB-SECOND (Target: <0.5s) - PASSED"))
            else:
                print(red("  Performance Check: SLOW (>0.5s) - WARNING"))
        else:
