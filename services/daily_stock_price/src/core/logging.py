...
def setup_logger(log_dir: str = "logs", app_name: str = "app"):
    ...
    # Loguru will automatically clean up older files matching the pattern.
    log_file_pattern = os.path.join(log_dir, f"{app_name}_{{time:YYYYMMDD_HHmmss}}.jsonl")
    logger.add(
        log_file_pattern,
        level="DEBUG",
        serialize=True,
        rotation="10 MB",  # Also rotate if a single run exceeds 10MB
    ...
    # 3. Isolated Error Log (JSONL) - Fixed name for easy access, but also rotated
    ...
    logger.add(
        ...
        serialize=True,
        rotation="10 MB",
        retention=5,  # Keep a bit more history for errors
        enqueue=True,
        backtrace=True,
        diagnose=True,
        encoding="utf-8",
