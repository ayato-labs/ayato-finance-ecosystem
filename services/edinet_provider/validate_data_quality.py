...
def validate_data():
    ...
    try:
        with db_manager.connect_master(read_only=True) as conn:
            ...
            recent_filings = conn.execute(
                "SELECT doc_id, filer_name, doc_description, submit_datetime FROM registry_db.filings ORDER BY submit_datetime DESC LIMIT 5"
            ).fetchall()
            ...
            sample_facts = conn.execute(
                "SELECT item_name, item_value, unit, fiscal_year FROM facts_db.company_facts LIMIT 5"
            ).fetchall()
            ...
            sample_narrative = conn.execute(
                "SELECT doc_id, section_name, content_md FROM narr_db.narratives LIMIT 1"
            ).fetchone()
            ...
            if sample_narrative:
                ...
                try:
                    ...
                    print(
                        f"  - Compression Status: OK (Compressed: {compressed_size} bytes -> Decompressed: {len(decompressed)} chars)"
                    )
