    ...
    try:
        with db_manager.connect_master(read_only=True) as conn:
            ...
            completed = conn.execute(
                "SELECT count(*) FROM ingestion_progress WHERE status = 'completed'"
            ).fetchone()[0]
            failed = conn.execute(
                "SELECT count(*) FROM ingestion_progress WHERE status = 'failed'"
            ).fetchone()[0]
            ...
            latest_date_row = conn.execute(
                "SELECT max(target_date) FROM ingestion_progress WHERE status = 'completed'"
            ).fetchone()
