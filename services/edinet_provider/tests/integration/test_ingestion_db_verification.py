...
class MockDoc:
    ...
    def parse(self):
        ...
        mock_report.text_blocks = {
            "Section 1": "This is a long enough narrative block for testing."
        }
        return mock_report
...
def test_ingestion_to_db_full_verification(tmp_path, monkeypatch):
    ...
    with db_manager.connect_master(read_only=True) as conn:
        ...
        filing = conn.execute(
            "SELECT doc_id, filer_name FROM registry_db.filings WHERE doc_id='ID_001'"
        ).fetchone()
        ...
        narrative = conn.execute(
            "SELECT section_name, content_md FROM narr_db.narratives WHERE doc_id='ID_001'"
        ).fetchone()
        ...
        conn.execute(
            "SELECT count(*) FROM facts_db.company_facts WHERE doc_id='ID_001'"
        ).fetchone()[0]
