...
def run_migration():
    ...
    with db_manager.connect_master(read_only=False) as conn:
        ...
        while True:
            ...
            for doc_id, sec_name, pre_decomp, comp_bytes, sess_id, ing_at in records:
                ...
                if pre_decomp is not None:
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Decompression failed for {doc_id}/{sec_name}: {dec_err}. Storing empty."
                        )
                        decompressed = ""
                else:
                    decompressed = ""
            ...
            logger.info(
                f"Progress: {progress:.1f}% ({offset:,}/{total_rows:,}). Elapsed: {elapsed / 60:.1f}m. Rate: {rate:.0f} rows/s. ETA: {eta:.1f}m"
            )
