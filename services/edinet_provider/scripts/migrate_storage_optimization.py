...
def migrate():
    ...
    tables = conn.execute(
        "SELECT table_name FROM information_schema.tables WHERE table_catalog = 'narr_db'"
    ).fetchall()
    ...
    if "narratives" in tables:
        ...
        column_type = conn.execute(
            "SELECT data_type FROM information_schema.columns WHERE table_catalog = 'narr_db' AND table_name = 'narratives' AND column_name = 'content_md'"
        ).fetchone()[0]
        if column_type == "VARCHAR":
            ...
            conn.execute(
                "CREATE TYPE narr_db.section_name_t AS ENUM (SELECT DISTINCT section_name FROM narr_db.narratives WHERE section_name IS NOT NULL)"
            )
            ...
            existing = conn.execute(
                "SELECT doc_id, section_name, content_md, session_id, ingested_at FROM narr_db.narratives"
            ).fetchall()
            ...
            for doc_id, section_name, compressed_blob, session_id, ingested_at in existing:
                try:
                    ...
                    decompressed_batch.append(
                        (doc_id, section_name, content, session_id, ingested_at)
                    )
                except Exception as e:
                    logger.warning(
                        f"Failed to decompress {doc_id} {section_name}: {e}. Storing as-is."
                    )
                    decompressed_batch.append(
                        (doc_id, section_name, str(compressed_blob), session_id, ingested_at)
                    )

            if decompressed_batch:
                conn.executemany(
                    "INSERT INTO narr_db.narratives_new VALUES (?, ?, ?, ?, ?)", decompressed_batch
                )
    ...
    conn.execute(
        "CREATE TYPE registry_db.form_code_enum AS ENUM (SELECT DISTINCT CAST(form_code AS VARCHAR) FROM registry_db.filings WHERE form_code IS NOT NULL)"
    )
    conn.execute(
        "CREATE TYPE registry_db.doc_type_code_enum AS ENUM (SELECT DISTINCT CAST(doc_type_code AS VARCHAR) FROM registry_db.filings WHERE doc_type_code IS NOT NULL)"
    )
    ...
    conn.execute(
        "CREATE TABLE facts_db.company_facts_temp AS SELECT * EXCLUDE (unit, fiscal_period), CAST(unit AS VARCHAR) as unit, CAST(fiscal_period AS VARCHAR) as fiscal_period FROM facts_db.company_facts"
    )

    # Fix encoding
    conn.execute(
        "UPDATE facts_db.company_facts_temp SET unit = '株' WHERE unit LIKE '%|%' OR unit = '株'"
    )
    conn.execute(
        "UPDATE facts_db.company_facts_temp SET unit = '円' WHERE unit LIKE '%~%' OR unit = '円'"
    )

    conn.execute(
        "CREATE TYPE facts_db.unit_enum AS ENUM (SELECT DISTINCT unit FROM facts_db.company_facts_temp WHERE unit IS NOT NULL)"
    )
    conn.execute(
        "CREATE TYPE facts_db.period_enum AS ENUM (SELECT DISTINCT fiscal_period FROM facts_db.company_facts_temp WHERE fiscal_period IS NOT NULL)"
    )
    ...
    tables = conn.execute(
        "SELECT table_name FROM information_schema.tables WHERE table_catalog = 'edinet_master' AND table_schema = 'main'"
    ).fetchall()
    tables = [t[0] for t in tables]
    if "ingestion_log" in tables:
        conn.execute(
            "CREATE TYPE main.ingestion_status_t AS ENUM ('PENDING', 'SUCCESS', 'PARTIAL_FAIL')"
        )
        conn.execute("""
    ...
    conn.execute(
        "INSERT OR REPLACE INTO migrations (name, applied_at) VALUES ('006_storage_optimization_final.py', CURRENT_TIMESTAMP)"
    )
